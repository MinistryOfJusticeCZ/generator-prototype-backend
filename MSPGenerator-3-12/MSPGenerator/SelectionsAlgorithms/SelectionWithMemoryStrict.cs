...
namespace SelectionsAlgorithms
{
    ...
    /// </summary>
    [AlgorithmDescription("Algoritmus:s pamětí2", Enabled = true, Description = "Přidělování s využitím aktuální vytíženosti (alpha=0.25, beta=4)", ID = 1)]
    public class SelectionWithMemoryStrict : SelectionAlgorithm
    {
        /// <summary>
        ...
        protected override void ComputeProbabilities(double[] loads, int[] activecases)
        {
            ...
            if (parray != null && loads.Length <= parray.Length)//test, zda je pole pravdepodobnosti alokovano a ma spravnou delku
            {
                if (aarray != null && loads.Length <= aarray.Length)//test, zda je pole aktualni vytizenosti alokovano a ma spravnou delku
                {
                    ...
                    else//v pripade, ze je aktualni  senaty aktualne vytizeny stejne, je naskalovano na hodnotu 1-nema to vliv na vyber senatu
                    {
