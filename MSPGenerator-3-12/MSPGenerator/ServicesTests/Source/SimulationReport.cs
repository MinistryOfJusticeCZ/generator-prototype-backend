...
namespace ServicesTests.HostedSimulationService //stejny jmenny prostor jako je service //DANO nahrad si to jmenym prostorem, ktery odkazuje na sluzby HostedServices.....
{
    /// <summary>
            ...
            using (SpreadsheetDocument spreadSheet = SpreadsheetDocument.Create(file, SpreadsheetDocumentType.Workbook))
            {
                ...
                foreach (var result in sresult)//pro kazdy algoritmus delej sheet
                {
                    ...
                    Sheet sheet = new Sheet()
                    {
                        ...
                        SheetId = id,
                        Name =alg_name /*ConvertAlgorithmName(alginfo[result.UsedAlgorithm].AlgorithmName)// chyba 2.3. KUB */
                    };
                    id++;
                    sheets.Append(sheet);
